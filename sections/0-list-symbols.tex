...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    $z_s,\ z_g$ & layout (semantic) and geometry latent subspaces \\
    $\beta$ & weight of the KL regulariser \\
    $K$ & number of Hilbert blocks (and geometry tokens) \\
    $g = \lceil N/K \rceil$ & primitives per block \\
    ...
    $d_{\mathrm{B}}(\cdot,\cdot)$ & Bures (2-Wasserstein) distance between covariances \\
  \end{tabular}
\end{table}
