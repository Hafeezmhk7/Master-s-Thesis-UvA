...
\paragraph{Completion.} Scene completion is posed as latent inpainting. A partial scan is encoded; the observed latent tokens are held fixed while the unobserved tokens are initialized from noise and denoised by the geometry model, conditioned on the (stable) coarse tokens; the frozen decoder then produces the completed scene. A deterministic layout code is advantageous here because the same partial scan yields the same conditioning. We specify these variants in full and set up the training, but, for the reasons developed in \autoref{sec:discussion}, do not bring completion to a finished, evaluated result; we treat it as the primary direction for future work and leave space for its results in \autoref{sec:results}.
...
\paragraph{Architecture and training.} The encoder and decoder use a transformer width of $384$ with $12$ attention heads, $6$ encoder and $12$ decoder layers, and an $8$-frequency Fourier embedding, following the inherited configuration. The latent uses $512$ tokens; the per-token width is the principal capacity knob. We train with AdamW, a base learning rate of $1\times 10^{-4}$ with cosine warm restarts and a short linear warmup, a small fixed KL weight, mixed bf16 precision, and distributed data parallelism across H100 GPUs. Full hyperparameters, the exact loss weights, and the dataset counts are listed in \autoref{sec:apx:first_appendix} for reproducibility.

\paragraph{Evaluation.} We evaluate reconstruction by per-attribute residual norms (position, colour, opacity, scale) reported on the same raw scale for training and validation so they are directly comparable, by the gauge-invariant covariance error for orientation and shape, and, for the final model, by image-space fidelity of renders of the reconstructed scenes (for example PSNR, SSIM, and LPIPS \citep{wang2004ssim,zhang2018lpips}); we leave space for these in \autoref{sec:results} as the final model finishes training. We measure generalization by the ratio of the held-out-full-scene error to the held-out-chunk error, where a ratio near one indicates no overfitting. We assess semantic structure qualitatively by principal-component visualizations of the per-primitive feature space and quantitatively by clustering and linear-probe measures on those features. Stage 2 generation and completion are evaluated by generation-quality and fidelity metrics for which we likewise leave space.